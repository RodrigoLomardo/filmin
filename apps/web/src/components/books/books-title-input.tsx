...
import { useBooksSearch } from '@/lib/hooks/use-books-search';
import { BooksResultItem } from './books-result-item';
import type { BookResult } from '@/types/book';

interface BooksTitleInputProps {
  value: string;
...
export function BooksTitleInput({ value, onChange, onSelect, required }: BooksTitleInputProps) {
  ...
  const { data: results = [], isFetching } = useBooksSearch(debouncedQuery);
  ...
  function handleSelect(result: BookResult) {
    ...
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        <div className="flex items-center">
          ...
          />
          {isFetching && (
            <Loader2 size={14} className="mr-4 flex-shrink-0 animate-spin text-amber-500" />
          )}
        </div>
      </div>

      {/* Dropdown de sugestões Google Books */}
      <AnimatePresence>
        {open && (
          <motion.div
